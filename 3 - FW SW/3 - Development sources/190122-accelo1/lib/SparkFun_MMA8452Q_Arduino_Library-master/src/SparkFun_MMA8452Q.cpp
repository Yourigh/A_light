...
byte MMA8452Q::init(MMA8452Q_Scale fsr, MMA8452Q_ODR odr)
{
	...
	//Wire.begin(); // Initialize I2C //JR change
...
void MMA8452Q::setupTap(byte xThs, byte yThs, byte zThs)
{
	...
	writeRegister(PULSE_CFG, temp | 0x40); //ELE on
...
byte MMA8452Q::readTap()
{
	...
	if (tapStat & 0x08) // Read EA bit to check if a interrupt was generated, single 0x80 mask, double 0x08 mask
	{
		return tapStat & 0x7F;
	}
	else

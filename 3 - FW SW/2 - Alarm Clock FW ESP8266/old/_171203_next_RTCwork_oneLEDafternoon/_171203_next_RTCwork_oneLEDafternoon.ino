...
#define ALARM_STEP_BRIGHTNESS_MS 100 //500?
  ...
  while (b == 20) {
    ...
    if (check_alarm()){
      ...
      if (millis()-last_update_time>1000){//every second
         ...
         if ((webtype == 0) && (WiFi.status() != WL_CONNECTED))  { //only for local Wifi Connection
            PRINTDEBUG("\nConnection lost, strying to reconnect");
    ...
    //Responses to time commands
      ...
      if (Alarm_Active == "A"){ 
        ...
        if (addDay){ //after 5AM, take it that you are setting the next day alarm, TEMPORARY
          Alarm[Alarm_ID-1] += 30;//86400; //add 24 hours thus 86400 seconds
        }
      }else{
...
void time_show(){
  ...
  for(byte l=0;l<24;l++){
    leds[l+SACRIFICIAL_LED].setRGB((4+(bright_on*96))*(int)minute_array[l],(3+(bright_on*96))*(int)minute_array[l],(3+(bright_on*16))*(int)second_array[l]);
  }
  FastLED.show();
}
...
bool check_alarm(){
  //simplest, checking if the instant in time is passed (alarm need to be set for specific date)
  return (Alarm[0]!=0)&&(now()>Alarm[0]);
}

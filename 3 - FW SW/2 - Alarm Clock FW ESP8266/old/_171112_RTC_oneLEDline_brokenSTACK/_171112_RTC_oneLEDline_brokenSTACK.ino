...
void LED_loading_anim(byte dot, byte strip_num) {
  ...
  byte led_begin = (strip_num ? NUM_LEDS_M-1+SACRIFICIAL_LED : SACRIFICIAL_LED); 
    //start at LED1 for hours and at LED 12 for minutes
...
void time_show(){
  ...
  byte led_begin_h = NUM_LEDS_M+SACRIFICIAL_LED; 
  byte led_begin_m = SACRIFICIAL_LED; 
  ...
  for(byte l=0;l<12;l++){
    leds[led_begin_h+l].setRGB(0*(int)hour_array[l],2*(int)hour_array[l],0);
  }

  for(byte l=0;l<24;l++){
    leds[led_begin_m+l].setRGB(1*(int)second_array[l],2*(int)minute_array[l],0);
  }
  FastLED.show();
}
...
void alarm_shine(){
  ...
  fill_solid( leds, NUM_LEDS_M+NUM_LEDS_H+SACRIFICIAL_LED, CRGB(br,br,br));
  FastLED.show();
  delay(100);
}

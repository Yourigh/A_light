...
void gettime() //get time from internet time server
{
  ...
  time_t now_server = time(nullptr); //declaration of variable now which is type time_t
  PRINTDEBUG(ctime(&now_server));
}
...
void time_show(){
  byte hour12h;
  hour12h = (now.Hour>11 ? now.Hour-12 : now.Hour); //12 hour time conversion
  bool hour_array[12];
  ...
  for(byte l=0;l<24;l++){
    minute_array[l] = !((now.Minute/2.5)<=l);
    second_array[l] = !((now.Second/2.5)<=l);
  }
  for(byte l=0;l<12;l++){
...
void time_update(){
  if (millis()-last_update_time>1000){ //every second
    ...
    if(now.Hour==24)  { now.Hour=0;   now.Date++;    } //not going on with over date stuff
    last_update_time = millis();
    time_show();
  }
}

...
void setup() {
  ...
  delay(100);
  //FastLED
  FastLED.addLeds<WS2812B, DATA_PIN_LED_H, GRB>(leds_h, NUM_LEDS_H);
  FastLED.addLeds<WS2812B, DATA_PIN_LED_M, GRB>(leds_m, NUM_LEDS_M);
  dot_H = 0;
...
//if returned 20, everything is ok, other value is bad
{
  ...
  else //web type 0 is when connected to local wifi
  {
    ...
    //Responses to time commands
      ...
      temp = req.substring(commaIndex[0]+1, commaIndex[0]+3);
      now_a.Month = temp.toInt();
      temp = req.substring(commaIndex[0]+4, commaIndex[0]+6);
      now_a.Date = temp.toInt();
      temp = req.substring(commaIndex[0]+9, commaIndex[0]+11);
      now_a.Year = temp.toInt();
      temp = req.substring(commaIndex[0]+9, commaIndex[0]+11);
      now_a.Year = temp.toInt();
      temp = req.substring(commaIndex[1]+1, commaIndex[1]+3);
      now_a.Hour = temp.toInt();
      temp = req.substring(commaIndex[1]+4, commaIndex[1]+6);
      now_a.Minute = temp.toInt();
      temp = req.substring(commaIndex[1]+7, commaIndex[1]+9);
      now_a.Second = temp.toInt();
      temp = req.substring(commaIndex[1]+10);
      now_a.DoW = temp.toInt();

      PRINTDEBUG("\nMonth:");
      PRINTDEBUG(now_a.Month);
      PRINTDEBUG("\nDate:");
      PRINTDEBUG(now_a.Date);
      PRINTDEBUG("\nYear:");
      PRINTDEBUG(now_a.Year);
      PRINTDEBUG("\nHourMinure:");
      PRINTDEBUG(now_a.Hour);
      PRINTDEBUG(now_a.Minute);
      PRINTDEBUG("\nDow:");
      PRINTDEBUG(now_a.DoW);
      PRINTDEBUG("\nSecond:");
      PRINTDEBUG(now_a.Second);
      ...
      String Alarm_ID_str = req.substring(commaIndex[0]+1, commaIndex[1]);
      String Alarm_Active = req.substring(commaIndex[1]+1, commaIndex[2]);
      String Alarm_Time_hour_str = req.substring(commaIndex[2]+1, commaIndex[2]+3);
      String Alarm_Time_minute_str = req.substring(commaIndex[2]+3, commaIndex[3]);
      ...
      int Alarm_ID = Alarm_ID_str.toInt();
      int Alarm_Time_hour = Alarm_Time_hour_str.toInt();
      int Alarm_Time_minute = Alarm_Time_minute_str.toInt();
      ...
      PRINTDEBUG("\nTime:");
      PRINTDEBUG(Alarm_Time_hour);
      PRINTDEBUG(":");
      PRINTDEBUG(Alarm_Time_minute);
      PRINTDEBUG("\nRepeat:");
...
void time_show(){
  byte hour12h;
  hour12h = (now_a.Hour>11 ? now_a.Hour-12 : now_a.Hour); //12 hour time conversion
  bool hour_array[12];
  ...
  for(byte l=0;l<24;l++){
    minute_array[l] = !((now_a.Minute/2.5)<=l);
    second_array[l] = !((now_a.Second/2.5)<=l);
  }
  for(byte l=0;l<12;l++){
    leds_h[l].setRGB(1*(int)hour_array[l],1*(int)hour_array[l],0);
  }
  for(byte l=0;l<24;l++){
    leds_m[l].setRGB(0,1*(int)minute_array[l],0*(int)second_array[l]);
  }
  FastLED.show();
}
...
void time_update(){
  if (millis()-last_update_time>1000){ //every second
    now_a.Second++;
    if(now_a.Second==60){ now_a.Second=0; now_a.Minute++; }
    if(now_a.Minute==60){ now_a.Minute=0; now_a.Hour++;   }
    if(now_a.Hour==24)  { now_a.Hour=0;   now_a.Date++;    } //not going on with over date stuff
    last_update_time = millis();
    time_show();
  }
}
